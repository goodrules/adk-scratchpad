...
def summarize_event(event) -> str:
    """Return a concise one-line summary of an ADK Event for logging."""
    parts = []
    ...
    if event.input_transcription:
        ...
        text = (t.text or "")[:80]
        parts.append(f"INPUT_TRANSCRIPTION({'partial' if event.partial else 'final'}): \"{text}\"")
    if event.output_transcription:
        t = event.output_transcription
        text = (t.text or "")[:80]
        parts.append(f"OUTPUT_TRANSCRIPTION({'partial' if event.partial else 'final'}): \"{text}\"")
    if event.content and event.content.parts:
        for p in event.content.parts:
            if p.text:
                parts.append(f"TEXT({len(p.text)} chars): \"{p.text[:80]}\"")
            if p.inline_data:
                size = len(p.inline_data.data) if p.inline_data.data else 0
                parts.append(f"AUDIO({p.inline_data.mime_type}, {size} bytes)")
            if p.function_call:
                parts.append(f"FUNC_CALL: {p.function_call.name}({list(p.function_call.args.keys()) if p.function_call.args else []})")
            if p.function_response:
                parts.append(f"FUNC_RESPONSE: {p.function_response.name}")
            if hasattr(p, 'executable_code') and p.executable_code:
                parts.append(f"EXEC_CODE({p.executable_code.language})")
            if hasattr(p, 'code_execution_result') and p.code_execution_result:
                parts.append(f"CODE_RESULT({p.code_execution_result.outcome})")
    if event.usage_metadata:
        um = event.usage_metadata
        prompt_tokens = getattr(um, 'prompt_token_count', '?')
        total_tokens = getattr(um, 'total_token_count', '?')
        parts.append(f"USAGE(prompt={prompt_tokens}, total={total_tokens})")
    author = getattr(event, 'author', '?')
    return f"[{author}] {' | '.join(parts) if parts else 'EMPTY_EVENT'}"


def check_for_truncation(event) -> str | None:
    """Check if a finished output transcription appears truncated."""
    if not event.output_transcription or event.partial:
        return None
    if not event.output_transcription.finished:
        return None
    text = (event.output_transcription.text or "").strip()
    if not text:
        return None
    truncation_patterns = [
        text.endswith(":"),
        text.endswith("following"),
        text.endswith("steps"),
        text.endswith("include"),
        text.endswith("are"),
    ]
    if any(truncation_patterns):
        return f"Response may be truncated — ends with: \"{text[-60:]}\""
    return None
...
async def websocket_endpoint(
    ...
    async def downstream_task() -> None:
        ...
        async for event in runner.run_live(
            ...
            logger.info(f"[STREAM] {summarize_event(event)}")
            truncation_warning = check_for_truncation(event)
            if truncation_warning:
                logger.warning(f"[TRUNCATION] {truncation_warning}")
