...
> **Note:** You must run from inside the `app` directory so Python can find the `rag_agent` module. Running from the parent directory will fail with `ModuleNotFoundError: No module named 'rag_agent'`.
